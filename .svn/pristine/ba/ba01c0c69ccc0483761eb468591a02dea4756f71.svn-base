<template>
    <div class="patriarch">
        ...
        <div v-if="step!=1">
            <div class="hello_img">
                <img style="width: 0.7rem;height: 0.7rem" :src="getStaticPath(info.icon)"
                     onerror="this.src='../../../../sasmobile/img/03.jpg'" alt="">
                <div class="stuName">{{info.name}}</div>
                <div class="school">
                ...
                <img class="pSwitch" style="width: 0.3rem" src="../../../../sasmobile/img/img99.png" @click="openShow"/>
                <div v-transfer-dom>
                    <div class="changeStudent">
                    <confirm v-model="show1" @on-confirm="change">
                        <ul>
                            <li>
                                <checklist :options="commonList" showSelf=true  title="切换学生" v-model="radioValue" :max="1"
                                           id="change1">
            ...
            <div class="hello_dynamic">
                <div @click="changes('pDynamic')"> <img style="height: 0.13rem;margin-right: 0.02rem " src="../../../../sasmobile/img/head_03.jpg"/>学习动态</div>
                <div @click="showConfirm=true">  <img style="height: 0.13rem;margin-right: 0.02rem" src="../../../../sasmobile/img/head_04.jpg"/>绑定更多学生</div>
            </div>
            ...
            <div class="homework ">
                ...
                <div v-for="(item,index) in dataWorkList" :key='item.index'>
                    ...
                    <div class="list-content" v-for='(item1,index1) in item' :key='item1.index1'>
                        <button v-if="item.isFinished" @click="openWork('homework',item1)" class="green" mini>已完成
                        </button>
                        ...
                        <ul class="ul">
                            ...
                            <li><span class="month">发布时间：</span><span
                                    class="name">{{item1.startTime | formatDate('MM月dd日 HH:mm')}}</span></li>
                        </ul>
                    </div>
                </div>
                <p class="btn" v-if="workTotalCount">
            ...
            <div class="homework ">
                ...
                <div v-for="(item,index) in dataTestList" :key='item.index'>
                    ...
                    <div class="list-content" v-for='(item1,index1) in item' :key='item1.index1'>
                        <button @click="openWork('test',item1)" v-if="item.answerStatus" class="green" mini>已完成</button>
                        <button @click="openWork('test',item1)" v-else class="red" mini>未完成</button>
                        <ul class="ul">
                            ...
                            <li><span class="month">发布时间：</span><span
                                    class="name">{{item1.releaseTime | formatDate('MM月dd日 HH:mm')}}</span></li>
                        </ul>
                    </div>
                </div>
                <p class="btn" v-if="testTotalCount ">
            ...
            <div class="homework">
                ...
                <div class="content" style="" v-for="(item,index) in productList" :key="index">
                    ...
                    <div class="homePage_book">
                        <div class="title">{{item.title}}
                            <div style="    padding-top: 0.04rem;" @click="openTo('recentPractice_content',item.id) ">
                                <span style="top: -0.04rem;position: relative;font-size: 0.14rem">查看进度</span>
                ...
                <p class="btn" v-if="productList.length">
    ...
    methods: {
      ...
      openWork (type, item) {
        this.$router.push({
          ...
          query: {groupsId: item.groupsId, seeType: 'parents'}
        })
      },
      connect () {
      ...
      queryGuardianStudent () {//查询家长学生关系
        this.ajax({
          ajaxSuccess: (data, res) => {
            ...
            if(typeof (this.$store.state.loginInfo.id)=='undefined'){
