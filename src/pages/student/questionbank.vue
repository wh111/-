...
<template>
    <div class="questionBank">
        ...
        <flexbox v-if="studentId">
            <flexbox-item :span="6/8"><x-input style="font-size: 0.1rem" placeholder="请输入搜索项" v-model="form.title"></x-input></flexbox-item>
        ...
        <div class="questionBankHeader" style="z-index: 100" :class="{'position':studentId}">
            <ul class="questionBank_nav">
                <li style="width: 25%">
                    ...
                    <!--<selector ref="defaultValueRef" placeholder="年级" title="" :options="gradeList" v-model="form.grade"></selector>-->
                    <!--<div class="active" @click="">年级</div>-->
                </li>
                <li style="width: 25%">
                    <!--<div class="active">科目</div>-->
        ...
        <div class="stance" v-if="!studentId"></div>
        <div style="clear: both;height: 0.5rem;background: #DEEAE1" v-if="!studentId"></div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      open (id) {
        this.$router.push({name: 'questionGradeContent',query:{id:id,studentId:this.studentId}})
      },
      getProductList () {
        let options = {
