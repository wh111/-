...
----------------------------------->
<template>
    <div class="tHomework" style="height: 100%">
        <header-title>
            <template
                    v-if="paperType=='homework'">布置作业
            </template>
            <template v-else>布置测评</template>
        </header-title>
        ...
        <div style="background: #fff">
            ...
            <ul class="tHomeworkSteps">
                <li :class="step>=0?'active':''">第一步</li>
                <li :class="step>=1?'active':''">第二步</li>
                <li :class="step>=2?'active':''">第三步</li>
            </ul>
        </div>
        <div style="background: #fff;margin-top: 0.2rem">
            ...
            <div v-if="step === 0">
                ...
                <flexbox>
                    <flexbox-item class="bolder" :span="6">科目</flexbox-item>
                    <div class="marginWord"> {{ userTextbookVersionInfo.subject | subject(self) }}</div>
                </flexbox>
                <flexbox>
                    <flexbox-item class="bolder" :span="6">教材</flexbox-item>
                    <div class="marginWord"> {{ userTextbookVersionInfo.textbookVersionName }}</div>
                </flexbox>
                <flexbox>
                    <flexbox-item class="bolder" :span="2">题型</flexbox-item>
                    <div class="innerText" style="">{{questionTypeList.toString()}} </div>
                    <div @click="showSelect('questionType')" style="height: 0.22rem" class="margin">

                        <x-icon type="ios-arrow-forward" size="0.22rem"></x-icon>
                    </div>
                </flexbox>
                <flexbox v-if="type=='homework'">
                    <flexbox-item class="bolder" :span="6">作业名称</flexbox-item>
                    <div>
                        <x-input placeholder-align="right" placeholder="输入名称" text-align="right"
                                 style="padding: 0 0.15rem;" v-model="formValidate.title"></x-input>
                    </div>
                </flexbox>
                <flexbox v-else>
                    <flexbox-item class="bolder" :span="6">试卷名称</flexbox-item>
                    <div>
                        <x-input placeholder-align="right" placeholder="输入名称" text-align="right"
                                 style="padding: 0 0.15rem" v-model="formValidate.title"></x-input>
                    </div>
                </flexbox>
                <group class="timeBorder timeBorderTimer" style="">
                    <datetime-range style="margin-top: 0;border-top: none" title="发布时间"
                ...
                <group class="timeBorder timeBorderTimer" style="margin-top: 0;border: none">
                    <datetime-range style="margin-top: 0;border-bottom: none" title="截至时间"
                                    :start-date="conductDate(new Date(), 'yyyy-MM-dd')"
                                    end-date="2030-02-02" format="YYYY-MM-DD HH:mm"
                                    v-model="submitTime" @on-change="onChange('submitTime')"></datetime-range>
                </group>
                <group class="timeBorder" style="margin-top: 0;border: none">
                    <datetime-range style="margin-top: 0;border-bottom: none" v-if="paperType !== 'homework'"
                                    end-date="2030-02-02" title="公布时间"
                                    :start-date="conductDate(new Date(), 'yyyy-MM-dd')"
                                    format="YYYY-MM-DD HH:mm" v-model="publishAnswerTime"
                                    @on-change="onChange('publishAnswerTime')"></datetime-range>
                </group>
                <!--<flexbox style="border-top: solid #DEEAE1 1.17647059em;">-->
                    <!--<flexbox-item class="bolder" :span="6">防舞弊设置</flexbox-item>-->
                    <!--<div>-->
                        <!--<selector ref="defaultValueRef" title="" :options="list"-->
                                  <!--style="height: 0.22rem;text-align: right" text-align="right" direction="rtl"-->
            ...
            <div v-if="step === 1">
                ...
                <!--label-width="100px">-->
                <flexbox>
                    <flexbox-item class="bolder" :span="6">年级</flexbox-item>
                    <div>
                ...
                <flexbox>
                    <flexbox-item class="bolder" :span="6">学期</flexbox-item>
                    <div>
                ...
                <flexbox>
                    <flexbox-item class="bolder" :span="6">章节</flexbox-item>
                    <div class="innerText">{{questionChapterList.toString()}}</div>
                    <div @click="showSelect('selectChapter')"  class="margin" style="height: 0.22rem">
                        <x-icon type="ios-arrow-forward" size="0.22rem"></x-icon>
                    </div>
                </flexbox>
                <flexbox>
                    <flexbox-item class="bolder" :span="6">知识点</flexbox-item>
                    <div class="innerText">{{questionKnowledge.toString()}}</div>
                    <div  class="margin" @click="showSelect('selectknowledge')" style="height: 0.22rem">
                        <x-icon type="ios-arrow-forward" size="0.22rem"></x-icon>
                    </div>
                </flexbox>
                <flexbox>
                    <flexbox-item class="bolder" :span="6">难度</flexbox-item>
                    <div>
                ...
                <!--</el-form-item>-->
                <flexbox style="background: #DEEAE1">
                    <flexbox-item :span="6" style="line-height: 0.3rem;height: 0.22rem;font-size: 0.14rem;color: #888">
                        试卷结构
                    </flexbox-item>
                </flexbox>
                <div style="width: 3.75rem;overflow: auto;margin-top: -0.02rem">
                    <x-table :cell-bordered="false" :content-bordered="false">
                        <thead>
                        <tr style="background-color: #F7F7F7;border-color: #EDF3EE;border-bottom: none">
                            <th class="bolder" style="text-align: left;padding-left: 0.15rem">知识点名称</th>
                            <td class="bolder">题型</td>
                            <td class="bolder">数量</td>
                        ...
                        <template v-for="(item,index) in questions">
                            <!--<tr></tr>-->
                            <td :rowspan="questionHeader.length+1"
                                style="text-align: left;padding-left: 0.15rem;width: 1.3rem;border-bottom: 0.01rem solid #dbdbdb">{{item.knowledgeName}}
                            </td>
                            <template v-for="(i,index1) in questionHeader">
                                <tr style="border-bottom: 0.01rem solid #dbdbdb">
                                    <td>{{i.label}}</td>
                                    <td style="width: 1.2rem;text-align: center">
                                        <template v-if="questions[index][i.prop]">
                                            <input v-model="questions[index][i.prop+'_edit']"
                                                   style="width: 0.34rem;height: 0.2rem;border-radius: 0.03rem;margin-top: 0.1rem;line-height: 0.18rem"></input>
        ...
        <div v-if="step === 2" style="background: #fff">
            <div style="width: 3.75rem;overflow: auto">
                <x-table :cell-bordered="false" :content-bordered="false">
                    <thead>
                    <tr style="background-color: #F7F7F7">
                        <th class="border">题型</th>
                        <th class="border">数量</th>
                        <template v-if="formValidate.scoreType === 1">
                            <th>单题分值</th>
                            <th>总分</th>
                        </template>
                        <th style="width: 1.8rem" class="border">知识点</th>
                    </tr>
                    </thead>
                    <tbody>
                    <tr style="border-bottom: 0.01rem solid #dbdbdb" v-for="(item,index) in questionsInfo">
                        <td>{{item.type}}</td>
                        <td>{{item.num}}</td>
                        <template v-if="formValidate.scoreType === 1">
                            <td><input style="width: 60%" v-model="questionsInfo[index].score"
                                       @input="setThisTotalScore(questionsInfo[index])"></input></td>
                            <td> {{item.totalScore}}</td>
                        </template>
                        <td>
                            <p style="margin-bottom: -0.01rem;border-bottom: 0.01rem solid #dbdbdb;text-align: left" v-for="(item1,index1) in item.knowledges" :key="index1">
            ...
            <div style="height: 0.4rem;background: #EDF3EE;padding-left: 0.15rem;line-height: 0.3rem;">
                <p style="font-size: 0.14rem;color: #888">使用试题库分数，并将总分折算为 100分</p>
            </div>
            ...
            <div style="height: 0.2rem;background: #EDF3EE"></div>
            <div class="qtt_chartMain">
                <!--<el-col :span="4">-->
                <!--<p class="qtt_centerText difNum">难度值：{{ (selectFormValidate.diffLevelCode || '全部') |-->
                <!--diffLevelCode(self)-->
        ...
        <div class="btn_wrap">
            ...
            <x-button class="greenButton" type="primary" v-if="step < 2" @click.native="next">下一步</x-button>
            <x-button class="greenButton" type="primary" v-if="step == 2" @click.native="preview">预览</x-button>
        </div>
  ...
  export default {
    data () {
      return {
        ...
        questionsInfoList: [],
        questionsInfoSelectList: [],//app选择试题
        // 接口
        // api: {
...
//      },
      ...
      onChange (type) {
        ...
        console.log(m, this.formValidate[type])
      },
      onConfirm () {
        if (this.confirmType == 'questionType') {
      ...
      init () {
        ...
        this.onChange('submitTime')
        this.onChange('publishAnswerTime')
        Util = this.$util
...
//              this.errorMess('试卷结构输入的题型数量必须是有效数字且不能大于总试题数量');
      ...
      next () {
        ...
        if (this.step == 1 && !this.questionsInfo.length) {
          // 检测第一步数据是否正确
        ...
        if (this.step == 2 && this.$router.query.type == 'intelligent') {
          this.message = '请输入答题时长'
          this.show = true
          return
        }
...
//    },
      ...
      serverToQuestions (arr) {
        ...
        arr.map(item => {
          ...
          console.log(temp[item.knowledgeId])
          temp[item.knowledgeId].total += item.total
        })
      ...
      getQuestionsInfoTable () {
        ...
        console.log(this.questionsInfo, '22')
//        this.questionsInfo.map()
//        this.questionsInfoSelectList
